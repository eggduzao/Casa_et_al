...
# Slurm Parameters
#SBATCH --job-name=MGP
#SBATCH --output=MGP.%A_%a.out
#SBATCH --error=MGP.%A_%a.err
#SBATCH --nodes=1
#SBATCH --mem=48gb
#SBATCH --time=5:00:00
...
# Input
inputFileName="/projects/ag-papan/eduardo/Wendt_Stag/Code/8_Meta_Plots_Genes/input/2_mgp.txt"
parameters=`sed "${SLURM_ARRAY_TASK_ID}q;d" $inputFileName`

# Execution
tempDir="/scratch/egusmao/MGP/"${SLURM_ARRAY_TASK_ID}"/"
mkdir -p $tempDir
cp /projects/ag-papan/eduardo/Wendt_Stag/Code/8_Meta_Plots_Genes/2_metaGenePlot.R $tempDir
cd $tempDir
