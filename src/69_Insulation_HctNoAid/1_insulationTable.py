...
def read_region_list(half_bin, resolution, region_file_name):
  ...
  for line in region_file:
    ...
    # Upstream region
    r1 = int(round_down(mid, resolution) - (half_bin * resolution))

    # Downstream region
    r2 = int(round_up(mid, resolution) + (half_bin * resolution))
...
def update_matrix(half_bin, resolution, matrix, region, matrix_dict):
  ...
  counterI = 0
  for i in range(int(region[1]), int(region[2]), int(resolution)):
    counterJ = 0
    for j in range(int(region[1]), int(region[2]), int(resolution)):
      try:
