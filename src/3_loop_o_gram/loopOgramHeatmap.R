...
createHeatmap <- function(data, nRegions, outputFile){
  ...
  hmbreaks = c(seq(0.0, 1.2,length=101))
  ...
  colkey(col = colorScheme, clim = c(0.0, 1.0), clab = NULL, clog = FALSE, add = TRUE, 
...
standardize <- function(data, minV = FALSE, maxV = FALSE){
  if(!minV){minV = min(data)}
  if(!maxV){maxV = max(data)}
  print(c(minV, maxV))
...
inputTable = standardize(log10(as.matrix(read.table(inputTableFileName, sep="\t", header=FALSE))), minV = -0.6378468, maxV = 1.1342134)
